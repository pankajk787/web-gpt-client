...
const UserInputContainer = ({ type, setMessages}) => {
  ...
  const askLLM = async (text) => {
    try {
      ...
      const { id, role, message } = data;
      setMessages((prev) => [
        ...prev,
        { id, role, text: message }
      ]);
    } catch(e) {
      console.log("Error", e);
    ...
    //   e.target.style.height = Math.max(e.target.scrollHeight, 350) + "px";
    // };
  return (
    <div className={`${type === "initial" ? "" : "fixed" } -bottom-2 w-[80%] mb-2 bg-neutral-900`}>
      <div className="chat-input-container mb-4 w-[80%] m-auto bg-neutral-950 rounded-xl">
        <textarea defaultValue={""} id="user-message res" autoFocus 
          ref={userInputRef} 
          rows="1" 
          className="w-full outline-0 p-2 resize-none overflow-hidden"
          // onInput={onInput}
          onKeyDown={onKeyDown}/>
        <div className="flex gap-2 justify-end mt-2 px-2">
          <button title="Web Search" className={`cursor-pointer`}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 20 20"
          ...
          <button title="Ask WebGPT" onClick={handleSend} disabled={loading} className="bg-slate-200 rounded-2xl text-gray-950 hover:text-gray-900 hover:bg-slate-100 px-3 text-lg cursor-pointer">
            {loading ? "Loading..." : "Send" }
          </button>
        </div>
        <p className="text-[10px] px-3 text-gray-400 py-0.5">
          Note: WebGPT can make mistakes.
        </p>
      </div>
    </div>
  );
